    ...
    <div className="p-6 space-y-6">
      ...
      <section className="space-y-4" aria-labelledby="live-agents-heading">
        ...
        {liveMerged.length === 0 ? (
          <p className="rounded-xl border border-slate-800 bg-slate-900/50 p-4 text-sm text-slate-400">
            No agents to show (no OpenClaw runtime entries and no mapped profiles).
          </p>
        ) : (
          <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
            {liveMerged.map((agent) => (
              <AgentCard key={agent.id} agent={agent} liveTelemetry />
            ))}
          </div>
        )}
      </section>
      ...
      {demoRoles.length > 0 ? (
        <section className="space-y-4" aria-labelledby="demo-roles-heading">
          ...
          <p className="text-sm text-slate-500">
            Demo / template roles with no OpenClaw mapping — not shown under live agents until you set{' '}
            <code className="rounded bg-slate-800 px-1 text-slate-300">openclawAgentId</code>.
          </p>
          <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
            {demoRoles.map((agent) => (
