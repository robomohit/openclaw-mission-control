...
export default function TasksPage() {
  ...
  const myQueueCount = tasks.filter(
    (task) =>
      task.assigneeType === 'user' && task.assigneeId === userAssigneeId,
  ).length;
  ...
  const filteredTasks = useMemo(
    () =>
      tasks
        .filter((task) => {
          if (boardTab === 'needs_review' && task.status !== 'review')
            return false;
          if (
            boardTab === 'mine' &&
            `user:${userAssigneeId}` !==
          ...
          if (
            filter.assignee &&
            `${task.assigneeType}:${task.assigneeId}` !== filter.assignee
          ) {
            return false;
          }
  ...
  const quickDispatch = [
    {
      ...
      description:
        'Queue a human approval item with the right defaults.',
      icon: ShieldCheck,
      defaults: {
        title: 'Review latest OpenClaw run',
        description:
          'Check output, validate safety, and unblock the next handoff.',
    ...
    },
    {
      ...
      defaults: {
        ...
        description:
          'Triage the issue, collect evidence, and post a concise summary back to Mission Control.',
    ...
    },
    {
      title: 'Recovery task',
      description:
        'Create an urgent response item for drift or runtime regressions.',
      icon: AlertTriangle,
      defaults: {
        title: 'Recover stalled automation',
        description:
          'Identify the blocker, restore the path to green, and note what changed.',
        assigneeId: defaultAgentId,
        assigneeType: 'agent' as const,
      ...
      <div className="space-y-6">
        <div className="flex flex-wrap items-center justify-between gap-4">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <select
              ...
              onChange={(e) =>
                setFilter((f) => ({ ...f, assignee: e.target.value }))
              }
            >
            ...
            <button
              type="button"
              onClick={() =>
                setFilter({ status: '', assignee: '', priority: '' })
              }
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-4">
          ...
          <div className="rounded-xl border border-amber-500/20 bg-amber-500/5 p-4">
            ...
            <p className="mt-1 text-sm text-slate-400">
              Human-in-the-loop sign-offs waiting.
            </p>
          </div>
          <div className="rounded-xl border border-rose-500/20 bg-rose-500/5 p-4">
            ...
            <p className="mt-1 text-sm text-slate-400">
              Tasks that have slipped past due date.
            </p>
          </div>
        ...
        <section className="rounded-xl border border-slate-800 bg-slate-900/50 p-6">
          <div className="mb-4 flex items-center justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-500">
                Common mission-control task templates for approvals, dispatch,
                and recovery.
              </p>
            </div>
          </div>
