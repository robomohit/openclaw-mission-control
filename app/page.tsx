...
import { buildSevenDayMetricSeries, formatDayLabel, useStore } from '@/lib/store';
import { StatCard } from '@/components/StatCard';
import { ActivityFeed } from '@/components/ActivityFeed';
import { useMemo } from 'react';
import { formatShortDate } from '@/lib/store';
import { AlertTriangle, CalendarClock, ShieldAlert, Users, Zap } from 'lucide-react';
import { useOpenClawStatus } from '@/lib/useOpenClawStatus';

export default function Home() {
  const { stats, tasks, calendarEvents, user, mission, agents, statsHistory, projects } = useStore();
  ...
  const offlineLiveCount = liveAgents.filter((a) => a.status === 'offline').length;
      ...
      },
      {
        ...
        detail:
          stalledProjects.length > 0
            ? stalledProjects.map((project) => project.name).slice(0, 2).join(' · ')
            : 'All tracked projects are on track',
      },
    ],
    [offlineLiveCount, overdueJobs.length, overdueTasks, reviewTasks.length, stalledProjects],
  );
  ...
  const recommendedMoves = useMemo(() => {
    ...
    if (moves.length === 0) {
      moves.push({
        ...
        detail: 'No urgent alerts. Use Tasks to queue the next batch of work.',
        href: '/tasks',
      });
    }
    ...
    <div className="flex flex-col lg:flex-row gap-6">
        ...
        <div className="grid grid-cols-1 gap-6 xl:grid-cols-[1.4fr_1fr]">
          <section className="rounded-xl border border-slate-800 bg-slate-900/50 p-6">
            <div className="mb-4 flex items-center justify-between">
              <div>
                <h2 className="font-semibold text-slate-100">Command queue</h2>
                <p className="mt-1 text-sm text-slate-500">
                  The mission-control layer: approvals, runtime alerts, and recovery work.
                </p>
              </div>
              <span className="rounded-full border border-slate-700 bg-slate-950 px-3 py-1 text-xs text-slate-400">
                {commandQueue.reduce((sum, item) => sum + item.count, 0)} open signals
              </span>
            </div>
          ...
          <section className="rounded-xl border border-slate-800 bg-slate-900/50 p-6">
            <div className="mb-4 flex items-start gap-3">
              ...
              <div>
                <h2 className="font-semibold text-slate-100">Recommended next moves</h2>
                <p className="mt-1 text-sm text-slate-500">
                  A simple operator checklist based on live state and local planning data.
                </p>
              </div>
            </div>
            <ul className="space-y-3">
              {recommendedMoves.map((move, index) => (
                <li key={move.title} className="rounded-lg border border-slate-800 bg-slate-950/50 p-3">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="min-w-0">
                      <div className="text-sm font-medium text-slate-100">{move.title}</div>
                      <p className="mt-1 text-sm text-slate-400">{move.detail}</p>
                      <a href={move.href} className="mt-2 inline-flex text-xs font-medium text-sky-400 hover:underline">
                        Open {move.href === '/tasks' ? 'task board' : move.href === '/projects' ? 'projects' : move.href === '/calendar' ? 'calendar' : 'team'}
                      </a>
                    </div>
                  </div>
                </li>
      ...
      <aside className="w-full lg:w-80 xl:w-96 space-y-6">
        ...
        <div className="rounded-xl border border-slate-800 bg-slate-900/50 p-4">
          <h2 className="mb-3 font-semibold text-slate-100">Operator watchlist</h2>
          <div className="space-y-3 text-sm">
            ...
            <div className="flex items-start gap-3 rounded-lg border border-slate-800 bg-slate-950/50 p-3">
              ...
              <div>
                <div className="font-medium text-slate-200">Live coverage</div>
                <p className="text-slate-500">
                  {gatewayHealth === 'up'
