                ...
                {upcomingEvents.map(e => (
                  <div key={e.id} className="rounded-lg border border-slate-700/60 bg-slate-800/50 p-4">
                    <div className="flex items-center justify-between">
                      <span className="font-medium text-slate-200 truncate">{e.title}</span>
                      <span className={`shrink-0 text-xs px-2 py-1 rounded-full ${e.confirmed ? 'bg-green-500/15 text-green-400' : 'bg-slate-700 text-slate-300'}`}>
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                {agents.map(a => (
                  <div key={a.id} className="flex items-center gap-3 rounded-lg border border-slate-700/60 bg-slate-800/30 p-3">
                    <div className="h-8 w-8 shrink-0 rounded-full flex items-center justify-center text-white text-xs font-bold" style={{ backgroundColor: a.avatarColor }}>
                      {a.name.charAt(0)}
                    </div>
                    <div className="min-w-0">
